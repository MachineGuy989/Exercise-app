...
def sound_check_card(p):
    ...
    n = len(tone_counts(page))
    page.click('[data-fid=sound-test]')
    ok(len(tone_counts(page)) == n, 'Test sound is silent while sound is OFF: %s' % tone_counts(page))
    ok('OFF' in page.inner_text('.toast'), 'and it says why')
    ...
    # sound switched off on the Data screen also silences a timer that runs to the end
    n = len(tone_counts(page))
    s.tab('today'); page.click('[data-fid=go-warmup]')
    ok('Off' in page.inner_text('[data-fid=g-mute]'), 'timer screen shows sound is off')
    page.clock.run_for(121000)
    ok(len(tone_counts(page)) == n, 'no beeps or chime while sound is OFF: %s' % tone_counts(page))
    ok(page.locator('.guided.timesup').count() == 1, "Time's up still flashes")
...
def service_worker_scope_and_offline(p):
    ...
    ok(caches == ['mikeExercise-v3'], 'cache names %s' % caches)
    keys = page.evaluate("()=>caches.open('mikeExercise-v3').then(c=>c.keys()).then(k=>k.map(r=>new URL(r.url).pathname.split('/').pop()||'./'))")
